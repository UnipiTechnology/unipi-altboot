...
if cd /tmp/__root/etc/systemd/system/multi-user.target.wants 2>/dev/null; then
    rm -f cacert-update.service cacert-update.timer merviscloudaccess.service mervisconfigtool.service mervisfcgi.service mervisnetserver.service mervisrt.service 
fi
